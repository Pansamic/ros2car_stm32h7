                                    ...
                                    const ip_addr_t *addr, u16_t port)
{
  ...
  {
    pbuf_free(p);
    LOG_ERROR("PTPD: event port queue full\n");
  }
}
                                      ...
                                      const ip_addr_t *addr, u16_t port)
{
  ...
  {
    pbuf_free(p);
    LOG_ERROR("PTPD: general port queue full\n");
  }
}
...
bool ptpd_net_init(NetPath *net_path, PtpClock *ptp_clock)
{
  ...
  {
    LOG_CRITICAL("PTPD: failed to find interface address\n");
    goto fail01;
  }
  ...
  {
    LOG_CRITICAL("PTPD: failed to create event UDP PCB\n");
    goto fail02;
  }
  ...
  {
    LOG_CRITICAL("PTPD: failed to create general UDP PCB\n");
    goto fail03;
  }
  ...
  {
    LOG_CRITICAL("PTPD: failed to encode multi-cast address: %s\n", addr_str);
    goto fail04;
  }
  ...
  if (!inet_aton(addr_str, &net_addr))
  {
    LOG_CRITICAL("PTPD: failed to encode peer multi-cast address: %s\n", addr_str);
    goto fail04;
  }
...
static ssize_t ptpd_net_recv(octet_t *buf, TimeInternal *time, BufQueue *queue)
{
  ...
  {
    LOG_ERROR("PTPD: received truncated packet\n");
    pbuf_free(p);
    return 0;
  }
  ...
  if (p->tot_len == 0)
  {
    LOG_ERROR("PTPD: received empty packet\n");
    pbuf_free(p);
    return 0;
  }
...
static ssize_t ptpd_net_send(const octet_t *buf, int16_t  length, TimeInternal *time, const int32_t * addr, struct udp_pcb * pcb)
{
  ...
  if (NULL == p)
  {
    LOG_ERROR("PTPD: failed to allocate transmit protocol buffer\n");
    goto fail01;
  }
  ...
  if (ERR_OK != result)
  {
    LOG_ERROR("PTPD: failed to copy data into protocol buffer (%d)\n", result);
    length = 0;
    goto fail02;
  }
  ...
  if (ERR_OK != result)
  {
    LOG_ERROR("PTPD: failed to send data (%d)\n", result);
    length = 0;
    goto fail02;
  }
